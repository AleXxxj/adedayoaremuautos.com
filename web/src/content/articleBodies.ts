 ...
 * bullets, throwing away the vehicle cards, captioned photographs, spec grids,
 * pros-and-cons panels, price tags, comparison rows and checklists.
 *
 * The body stops where the share row begins. The legacy pages nest the share
 * row, author box, related posts and comment thread inside `.blog-content`,
 * but the route renders those itself — with share links that work, related
 * links that resolve, and comments that are real and moderated — so leaving
 * them in the body showed every one of them twice.
 *
 * These are the owner's own pages, held in this repository rather than user
 * input, so they are stored as markup. Only image paths are rewritten.
 */
